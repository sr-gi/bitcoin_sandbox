...
from bitcoin_sandbox.ln_node import LN_Node
...
def create_ln_scenario_from_graph(btc_containers, btc_addrs, g):
    ...
    container_values = list(btc_containers.values())
    ln_nodes_info = get_ln_nodes_info(container_values, btc_addrs)
    ...
    spread = False
    while not spread:
        mempools = [bitcoin_cli.getrawmempool(container) for container in container_values]
    ...
    bitcoin_cli.generatetoaddress(container_values[0], 6, list(btc_addrs.values())[0])
...
def build_simulation_env(client):
    ...
    btc_containers = {
        ...
        if DOCK_CONTAINER_NAME_PREFIX in container.name
    }

    # Give every node some bitcoins to start with
    container_values = list(btc_containers.values())
    btc_addrs = create_onchain_setup(container_values)

    # Wait until lnd has processed all new blocks so funds show up in every node
    wait_until_mature(container_values)
