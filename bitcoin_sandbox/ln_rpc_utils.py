...
def lightning_rpc(container, command, params=None, verbose=False):
    if params is None:
        params = []
...
def connect(container, peer_id, peer_ip, peer_port, verbose=False):
    return lightning_rpc(container, "connect", "{}@{}:{}".format(peer_id, peer_ip, peer_port), verbose=verbose).get(
        "id"
    )
...
def sendtoroute(container, payment_hash, route, verbose=False):
    return lightning_rpc(
        container, "sendtoroute", ["--pay_hash={}".format(payment_hash), "-r='{}'".format(route)], verbose=verbose
    )
